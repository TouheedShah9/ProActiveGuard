...
def get_shap(models_dict, state, w, wbgt, risk):
    ...
    try:
        ...
        for i,f in enumerate(models_dict["features"]):
            base=f.split("_lag")[0].split("_rm")[0].split("_slope")[0].split("_rstd")[0]
...
def get_rl_action(rl_agent, risk, wbgt, accl, minutes):
    ...
    if rl_agent:
        try:
            rb=min(int(risk/20),4); wb=min(int((wbgt-25)/5),4)
            ab=0 if accl<7 else 1 if accl<14 else 2; hb=min(int(minutes/120),3)
            aid=int(np.argmax(rl_agent.q_table[(rb,wb,ab,hb)]))
...
def refresh_rate(speed):
    ...
    A live monitoring dashboard should redraw often enough to feel
    "live" but slowly enough that a person can actually see what
    changed. "Sim Speed" still does its job: a higher speed means
    more simulated minutes advance between each of these (still calm)
    redraws, not that the screen flickers faster.

    Mobile gets a slightly longer interval than desktop (3.5s vs
    2.5s) — real phone testing showed the ~10+ Plotly charts redrawn
    each tick need a bit more breathing room on typical mobile
    hardware than on desktop.
    """
    return 3.5 if is_mobile() else 2.5
