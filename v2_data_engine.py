...
# ══════════════════════════════════════════════════════════════
def pennes_temp_delta(T_core, T_skin, metabolic_watts, dt=60):
    """
...
def generate_physiological_signals(worker, t_min, wbgt, solar_rad,
    ...
    delta_T    = pennes_temp_delta(T_core_prev, T_skin_est, met_w, dt=60)
...
def build_dataset():
    ...
    for worker in WORKERS:
        ...
        # Per-worker signal history for fault detection
        hr_history  = [None, None]
        ct_history  = [None, None]
        ...
        for i, ts in enumerate(timestamps):
            ...
            for sig in ["heart_rate_bpm", "hrv_rmssd_ms",
                ...
                prev  = hr_history[0] if sig == "heart_rate_bpm" else None
                prev2 = hr_history[1] if sig == "heart_rate_bpm" else None
            ...
            # Update history
            hr_history = [signals["heart_rate_bpm"], hr_history[0]]
            ct_history = [signals["core_temp_c"],    ct_history[0]]
